...
export const deletJuego = async(req,res) =>{
    try{
      const {id} = req.params
      await eliminarImagen(id)
...
const eliminarImagen = async(id) =>{
 ...
 const validar = (nombre,niveles,fecha,img,sevalida) =>{
    ...
    if(niveles === undefined || niveles.trim() === '' || isNaN(niveles)){
        errors.push('El numero de niveles NO debe de estar vacio y debe de ser numerico')
    }
    if(fecha === undefined || fecha.trim() === '' || isNaN(Date.parse(fecha))){
        errors.push('El fecha NO debe de estar vacio y debe de ser fecha valida')
    }
